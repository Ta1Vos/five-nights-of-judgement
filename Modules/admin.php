...
function includeAdminToCards(string $type = "", $id = null): false|string {
    ...
    if (isAdmin() && $params[1] === "admin" && !empty($type) && isset($id)) {
        ...
        switch ($type) {
            case "product":
                return "<a href='/admin/edit/product/$id' class='z-5 text-decoration-none text-black'><button>Edit product</button></a><a href='/admin/delete/product/$id' class='z-5 text-decoration-none text-black'><button>Edit product</button></a>";
